...
from nd_robotic_ai.platform.ros.message.kind.sensor.lidar.scan.scan import Scan as RosScanMessage
from nd_robotic_ai.platform.ros.message.kind.sensor.nav.odometry import Odometry as RosOdometryMessage
from nd_robotic_ai.robot.part.mind.part.cognition.part.object_level.process.part.memory.action.kind.intra.segregating.kind.slicing.kind.by_feature.feature import Feature as FeatureSlicer
...
from nd_robotic_ai.robot.part.mind.part.cognition.part.object_level.process.part.memory.composition.leaf.leaf import \
    Group as TraceGroup
from nd_robotic_ai.robot.part.mind.part.cognition.part.object_level.process.part.memory.decorator.storaged.storaged import \
    Storaged
from nd_utility.data.kind.dic.dic import Dic
from nd_utility.data.kind.slice.slice import Slice as UtilitySlice
...
class RosBagYamlMessageSegragating(FeatureSlicer, TraceAddValueSubscriber, GroupRamValuesAdditionFinishedSubscriber):
    ...
    def do_when_a_new_value_is_added_to_ram(self, value: Any) -> None:
        """
        Is called from SlicedValues internal_storage decoration
        """
    ...
    def do_when_group_ram_values_addition_is_finished(self) -> None:
        ...
        for trace_kind_name, trace_group in self._trace_groups_dic.get_keys_values():
            ...
            current_uni_format_yaml_file_parent_storage_str_path = Path(
                current_umvyf_storage_str_path).get_parent_directory_string_path()

            segregated_storage_dir_path = DirPath(
                current_uni_format_yaml_file_parent_storage_str_path + os_sep + trace_group.get_name() + os_sep + trace_group.get_name())

            segregated_storage_file_path = FilePath(
                segregated_storage_dir_path.get_native_os_string_path_with_trailing_slash() + "." + self._storage_type_after_segregation)

            os_file = OsFile(segregated_storage_file_path, None, None)
            # TODO here multi valued must be replaced with Sliced
            trace_group_storage = UniKinded(
                Sliced(PklDataStorage(os_file, create_directory_structure=True), self._slice, None), trace_kind_name,
                False)
