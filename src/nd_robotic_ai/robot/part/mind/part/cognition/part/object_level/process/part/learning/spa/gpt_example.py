# =======================================================
# Perception–RotorComposite–Act (SPA) skeleton with explicit Protocols
...
class ActionProto(Protocol):
    """Atomic executable overlap_size (may call ROS actions/services)."""
...
@dataclass
class WorldState:
    """Fused state from Perception overlap_size (fill from ROS topics)."""
    pose: Any
    obstacles: Any
    battery: float
    risk_level: float
...
class GoToWaypoint(ActionProto):
    ...
    def expected_cost(self, state: WorldState) -> float:
        # Replace with distance/time_based/energy estimate
        return 5.0
    ...
    def execute(self, state: WorldState) -> WorldState:
        # TODO: send ROS role/service/action; wait/result handling
        return state
...
class SPAController:
    """Perception → RotorComposite → Act loop with pre_plan retention and re-evaluation."""
    ...
    def sense(self) -> WorldState:
        # TODO: fuse ROS topics (odom, sensory, battery, etc.)
