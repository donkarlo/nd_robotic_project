...
from nd_robotic.robot.action.kind.suprise_reducing.suprise_reducing import SupriseReducing
...
class BuildingSingleModalityForcastingModelsFromEpisodes(SupriseReducing):
    """
    ...
    def __init__(self) -> None:
        ...
        goal = SuprisePoise(desired_suprise_state, acceptance)

        Action.__init__(self, goal)

    def build_forcasting_modalities(self)->None:
        episodes = Robot().get_episodes()
        forcasting_model_architecture = Robot().get_priming_forcasting_models_architecture()
        forcasting_model_configs = Robot().get_priming_forcasting_models_configs()
