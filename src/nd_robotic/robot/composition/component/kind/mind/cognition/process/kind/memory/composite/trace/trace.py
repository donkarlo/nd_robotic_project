...
from nd_robotic.robot.composition.component.kind.mind.cognition.process.kind.memory.composite.trace.interface import Interface as TraceInterface
from nd_robotic.robot.composition.component.kind.mind.cognition.process.kind.memory.composite.trace.kind.core.kind import Kind as TraceKind
...
class Trace(TraceInterface):
    ...
    - trace is the connection between body's nervous_system system and mind. on one hand it receives potential activity fieelds and on theother hand it converts it to a nested action_potential_group point such as a dictionary or a composition pattern
    ...
    - action_potentials can not be decorated or be composition or decorated
    - trace covers from action_potential activit field to structed action_potential_group
    - this is the inner_experiment of composition pattern
    """
    ...
    def __init__(self, formatted_data:Optional[Any], action_potential_group: Optional[ActionPotentialGroup] , name: Optional[str]):
        ...
        # is it a action? a lidar sensor? a initial_mission? This must be guessed from formatted action_potential_group
